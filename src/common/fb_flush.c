...
void fb_flush_init(void *internalBuffer, size_t bufSize, lv_display_flush_cb_t ll_flush_cb, uint8_t can_flush_ready, uint8_t x_coord_align, uint8_t y_coord_align)
{
...
void fb_flush_callback(lv_display_t *drv, const lv_area_t *area, uint8_t *px_map)
{
    ...
    if ((s_flush.x_coord_align > 1) ||
        (s_flush.y_coord_align > 1))
    {
    ...
    if (((width >= flush_width) && (height >= flush_height)) || flushAreaTooBig) // same size or too big
    {
        if (flushAreaTooBig)
        {
            beelog_print(BEELOG_LEVEL_WARN, __FILE__, "area to flush is bigger than internal buffer %dx%d", flush_width, flush_height);
        }
    ...
    else
    {
        ...
        // beelog_print(BEELOG_LEVEL_DEBUG, __FILE__, "r(%d:%d) -> (%d:%d) %dx%d", flush_area.x1, flush_area.y1, flush_area.x2, flush_area.y2, flush_width, flush_height);
        ...
        for (int32_t y = 0; y < flush_height; y++)
        {
            if (y < height)
            {
                ...
                // beelog_print(BEELOG_LEVEL_DEBUG, __FILE__, "dst_data %p", dst_data);
            ...
            else
            {
                ...
                // beelog_print(BEELOG_LEVEL_DEBUG, __FILE__, "pLastLine %p", pPrevLine);
