...
void Dashboard::populate()
{
    ...
    // Current Time
    timeLabel = lv_label_create(getCurrentScreen());
    updateCurrentTime();
    ...
    etaLabel = lv_arclabel_create(getCurrentScreen());
    ...
    edaLabel = lv_arclabel_create(getCurrentScreen());
    ...
    // Direction Icon
    directionIcon_m.init();
    directionIcon_m.setPosition(LV_ALIGN_CENTER, 0, -40);
    directionIcon_m.setScale(512);
    updateNextInstructionIcon();
    connect(directionIcon_m.getObj(), NavigationModel::NavigationEvents::EVENT_NEXT_INSTRUCTION_ICON_UPDATED, (lv_event_cb_t) &Dashboard::updateNextInstructionIcon, this);

    // Direction label
    directionLabel = lv_label_create(getCurrentScreen());
    ...
    directionDistanceLabel = lv_label_create(getCurrentScreen());
    lv_obj_set_style_text_align(directionDistanceLabel, LV_TEXT_ALIGN_CENTER, 0);
    updateDirectionDistanceLabel();
    connect(directionDistanceLabel, NavigationModel::NavigationEvents::EVENT_REMAINING_DISTANCE_UPDATED, &updateDirectionDistanceLabel);
    ...
    connectionStateLabel = lv_label_create(getCurrentScreen());
...
{
    BeelightApp::instance()->navigateTo<Dashboard>(NavigationTransition::TO_RIGHT, true);
}
